...
uint8_t sx127x_get_syncword_length(const sx127x_t *dev)     /*_APS_OSS7_ import */
{
    switch (dev->settings.modem) {
        case SX127X_MODEM_FSK: {
...
void sx127x_set_syncword_length(sx127x_t *dev, uint8_t len) /*_APS_OSS7_ import */
{
...
uint8_t sx127x_get_syncword(const sx127x_t *dev, uint8_t *syncword, uint8_t sync_size)  /*_APS_OSS7_ refactor */
{
    /* refactor: FSK mode has a variable syncword length, LORA has fixed 1 byte syncword */
    uint8_t size;
...
void sx127x_set_syncword(sx127x_t *dev, uint8_t *syncword, uint8_t sync_size)   /*_APS_OSS7_ refactor */
{
    /* refactor: FSK mode has a variable syncword length, LORA has fixed 1 byte syncword */
    assert(sync_size >= 1);
...
/*#ifdef PLATFORM_SX127X_USE_VCC_TXCO
    ...
    // disable the SPI? spi_disable(spi_handle);
}
    ...
    switch (dev->settings.modem) {
        case SX127X_MODEM_FSK:
            ...
            else
            {
                ...
                gpio_irq_enable(dev->params.dio0_pin); // enable the PayloadReady interrupt
            }
    ...
    switch (dev->settings.modem) {
        case SX127X_MODEM_FSK:
            if (dev->settings.fsk.rx_timeout != 0) {
                xtimer_set(&(dev->_internal.rx_timeout_timer), dev->settings.fsk.rx_timeout);
            }
...
void sx127x_set_tx(sx127x_t *dev)
{
    ...
    if ((dev->settings.modem == SX127X_MODEM_LORA) && (dev->settings.lora.tx_timeout != 0)) {
...
static void set_antenna_switch(opmode_t opmode) {
  if(opmode == OPMODE_TX) {
    ...
    gpio_set(SX127x_MANUAL_RXTXSW_PIN);
#endif
#ifdef PLATFORM_USE_ABZ
    gpio_clear(ABZ_ANT_SW_RX_PIN);
    if((read_reg(REG_PACONFIG) & RF_PACONFIG_PASELECT_PABOOST) == RF_PACONFIG_PASELECT_PABOOST) {
      gpio_clear(ABZ_ANT_SW_TX_PIN);
      gpio_set(ABZ_ANT_SW_PA_BOOST_PIN);
    } else {
      gpio_set(ABZ_ANT_SW_TX_PIN);
      gpio_clear(ABZ_ANT_SW_PA_BOOST_PIN);
    }
#endif
  } else {
#ifdef PLATFORM_SX127X_USE_MANUAL_RXTXSW_PIN
    gpio_clear(SX127x_MANUAL_RXTXSW_PIN);
#endif
#ifdef PLATFORM_USE_ABZ
    gpio_set(ABZ_ANT_SW_RX_PIN);
    gpio_clear(ABZ_ANT_SW_TX_PIN);
    gpio_clear(ABZ_ANT_SW_PA_BOOST_PIN);
#endif
  }
...
void sx127x_set_bandwidth(sx127x_t *dev, uint32_t bandwidth) /* _APS_OSS7_ FSK bw is 32 bits */
{
    DEBUG("[sx127x] Set bandwidth: %u\n", (unsigned int)bandwidth);
