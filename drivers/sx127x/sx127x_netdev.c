...
static void dump_register(sx127x_t *dev)
{
    ...
    for (uint8_t add=1; add <= SX127X_REG_VERSION; add++)
    ...
    // To be more safe, the dump start after the FIFO register
...
#ifdef _APS_OSS7_ //FIXME: this breaks the lora module
    ...
    switch (dev->settings.modem) {
        case SX127X_MODEM_FSK:
            ...
            if (size > SX127X_FIFO_MAX_SIZE)
            {
                ...
                assert(gpio_init_int(dev->params.dio1_pin, GPIO_IN, GPIO_FALLING,
            ...
            else
            {
                ...
                {
                    sx127x_write_fifo(dev, iolist->iol_base, size);
                    assert(gpio_init_int(dev->params.dio0_pin, GPIO_IN, GPIO_RISING,
...
#ifdef _APS_OSS7_
    memcpy(&sx127x->params, sx127x_params, sizeof(sx127x_params_t));  /* FIXME: normally project specific */
#endif
...
static int _set_state(sx127x_t *dev, netopt_state_t state)
{
  ...
  if(state == NETOPT_STATE_SLEEP)
    gpio_clr(dev->params.vcc_txco);
  else
    gpio_set(dev->params.vcc_txco);
#endif
...
void _on_dio0_irq(void *arg)
{
    ...
    switch (dev->settings.state) {
...
#ifdef _APS_OSS7_
            sx127x_read_fifo(dev, dev->packet.buf, dev->packet.length); //FIXME???
#endif
        ...
        //FIXME: break introduced in FSK impl ?
                case SX127X_MODEM_FSK:
                default:
#ifdef _APS_OSS7_
