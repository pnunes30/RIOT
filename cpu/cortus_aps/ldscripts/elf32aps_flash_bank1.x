...
MEMORY
{
  ...
  /* Normally the first 32 bytes are unused (to avoid NULL pointer accesses
     ...
     its registers */
  program_memory_ota (rx) :  org = 0x8003FE00, len = 255K
...
SECTIONS
{
  ...
  .text   :
  {
    ...
    *(.text .text.* .libgcc .gnu.linkonce.t.*);
    ...
    . = ALIGN(4);
    PROVIDE(_etext = .);
  }  > program_memory AT > program_memory_ota :text
  ...
  .data    :
  {
    PROVIDE(_data_image = LOADADDR(.data) - 0x3FE00);
    PROVIDE(_sdata = .);
    *(.sdata);
    ...
    *(SORT_BY_ALIGNMENT(.gnu.linkonce.d.*));
    . = ALIGN(4);
  }  > data_memory AT > program_memory_ota :data
